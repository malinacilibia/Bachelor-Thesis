    ...
    <div class="container py-5 d-flex justify-content-center">
        <div class="card shadow-lg border-0 w-100" style="max-width: 700px; border-radius: 20px;">
            <div class="card-body p-5">
                <h2 class="mb-4 text-center fw-bold" style="color: #4e705a;">Editează povestea ta de adopție</h2>
                ...
                <form action="{{ route('story.update', $story->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-3" id="title" name="title" value="{{ $story->title }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="content" class="form-label fw-semibold">Povestea ta</label>
                        <textarea class="form-control rounded-3" id="content" name="content" rows="5" required>{{ $story->content }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label for="image" class="form-label fw-semibold">Schimbă poza (opțional)</label>
                        <input type="file" class="form-control rounded-3" id="image" name="image" accept="image/*">
                    </div>

                    @if($story->image)
                        <div class="mb-3 text-center">
                            <p class="mb-2 fw-semibold">Poză existentă:</p>
                            <img src="{{ asset('storage/' . $story->image) }}" alt="Poza existentă" class="img-fluid rounded" style="max-height: 200px;">
                        </div>
                    @endif

                    <div class="text-center">
                        <button type="submit" class="btn px-5 py-2 fw-bold" style="background-color: #cce5cc; color: #333; border-radius: 30px;">
                             Actualizează povestea
                        </button>
                    </div>
                </form>
            </div>
